...
export async function TrueSearch(token, number) {
  ...
  return axios.get("https://search5-noneu.truecaller.com/v2/search", {
    ...
    return obj.name = res.name, obj.score = res.score, obj.access = res.access,
      obj.e164Format = res?.phones?.[0]?.e164Format, obj.numberType = res?.phones?.[0]?.numberType,
      obj.nationalFormat = res?.phones?.[0]?.nationalFormat, obj.dialingCode = res?.phones?.[0]?.dialingCode,
      obj.countryCode = res?.phones?.[0]?.countryCode, obj.carrier = res?.phones?.[0]?.carrier,
      obj.type = res?.phones?.[0]?.type, obj.address = res?.addresses?.[0]?.address,
